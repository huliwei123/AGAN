...
class AGAN(object):
    ...
    def fitter(self,x, is_training=True, reuse=False):
        with tf.variable_scope("fitter", reuse=reuse):
            ...
            net = tf.nn.relu(bn(linear(net, 64 * 7 * 7, scope='f_fc2'), is_training=is_training, scope='f_bn2'))
            net = tf.reshape(net, [self.batch_size, 7, 7, 64])
            net = tf.nn.relu(
                bn(deconv2d(net, [self.batch_size, 14, 14, 8], 4, 4, 2, 2, name='f_dc3'), is_training=is_training,
                   scope='f_bn3'))
