...
class AGAN(object):
    ...
    def discriminator(self, x, is_training=True, reuse=False):
        ...
        with tf.variable_scope("discriminator", reuse=reuse):
            ...
            net = lrelu(bn(linear(net, 1024, scope='d_fc1'), is_training=is_training, scope='d_bn3'))
            out_logit = linear(net, 1, scope='d_fc2')
            out = tf.nn.sigmoid(out_logit)
            return out, out_logit, net

            # net = tf.nn.relu(
            #     bn(deconv2d(x, [self.batch_size, 56, 56, 4], 4, 4, 2, 2, name='d_dc1'), is_training=is_training,
            #    scope='d_bn1'))
            # net = tf.reshape(net,[self.batch_size,-1])
            # net = lrelu(bn(linear(net, 512, scope='d_fc1'), is_training=is_training, scope='d_bn3'))
    ...
    def fitter(self,x, is_training=True, reuse=False):
        with tf.variable_scope("fitter", reuse=reuse):
            ...
            # net = tf.nn.relu(bn(linear(net, 512, scope='f_fc1'), is_training=is_training, scope='f_bn1'))
            # net = tf.nn.relu(bn(linear(net, 128 * 7 * 7, scope='f_fc2'), is_training=is_training, scope='f_bn2'))
            # net = tf.reshape(net, [self.batch_size, 7, 7, 128])
            # net = tf.nn.relu(
            #     bn(deconv2d(net, [self.batch_size, 14, 14, 64], 4, 4, 2, 2, name='f_dc3'), is_training=is_training,
            #        scope='f_bn3'))
            # out = tf.nn.sigmoid(deconv2d(net, [self.batch_size, 28, 28, 1], 4, 4, 2, 2, name='f_dc4'))
            # return out
            #先采用卷积提取特征，然后反卷积生成图像
            net = lrelu(conv2d(x, 64, 4, 4, 2, 2, name='f_conv1'))
            net = lrelu(bn(conv2d(net, 128, 4, 4, 2, 2, name='f_conv2'), is_training=is_training, scope='f_bn2'))
            net = tf.nn.relu(
                bn(deconv2d(net, [self.batch_size, 14, 14, 64], 4, 4, 2, 2, name='f_dc3'), is_training=is_training,
                   scope='f_bn3'))
            net = tf.nn.sigmoid(deconv2d(net, [self.batch_size, 28, 28, 1], 4, 4, 2, 2, name='f_dc4'))
            return net
    ...
    def build_modle(self):
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            ...
            self.f_optim_2 = tf.train.AdamOptimizer(self.learning_rate * 5, beta1=self.beta1) \
                .minimize(self.f_loss_2, var_list=[f_vars])
                ...
                if epoch<5:
                    ...
                    _, d_loss = self.sess.run([self.d_optim, self.d_loss],
                                          feed_dict={self.inputsY: batch_images_Y,
                                                     ...
                                                     self.z: batch_z})
                    # update F network
                    _, f_loss = self.sess.run([self.f_optim_2, self.f_loss_2],
                                              feed_dict={self.inputsX: batch_images_X,
                                                         self.z: batch_z})
